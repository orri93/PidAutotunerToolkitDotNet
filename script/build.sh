...
GOS_CMAKE_CREATE_OPTIONS="${GOS_CMAKE_CREATE_OPTIONS} -DGOS_PID_TOOLKIT_EXTERNAL_PROJECT:BOOL=ON"
if [[ $GOS_BUILD_DOCS = "ON" ]]; then
  echo "Adding documentation options"
  GOS_CMAKE_CREATE_OPTIONS="${GOS_CMAKE_CREATE_OPTIONS} --graphviz=${GOS_PROJECT_ARTIFACTS_DIR}/share/graphviz/pidtoolkit"
  GOS_CMAKE_CREATE_OPTIONS="${GOS_CMAKE_CREATE_OPTIONS} -DBUILD_DOCS:BOOL=${GOS_BUILD_DOCS}"
else
  echo "Skipping documentation options"
fi
...
#GOS_CMAKE_CREATE_OPTIONS="${GOS_CMAKE_CREATE_OPTIONS} -A \"${GOS_CMAKE_PLATFORM}\""
