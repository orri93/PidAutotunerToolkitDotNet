...
ColumnLayout {
  ...
  signal numberMaximumChanged(real maximum)
  signal numberPrecisionChanged(real precision)
  signal numberStepSizeChanged(real stepSize)
  ...
  function setNumberPrecision(precision) {
    numberPrecisionSpinBox.value = precision;
  }
  function setNumberStepSize(stepSize) {
    numberStepSizeTextInput.text = stepSize.toString();
  }
